//! Cross-face corner handling for chunks at cube corners.
//!
//! A cube has 8 corners where 3 faces meet. When a chunk sits at a corner
//! of its face, it needs diagonal neighbors on the other two faces. This
//! module defines the corner topology and provides neighbor-finding functions.
...
/// Which UV corner of a face a chunk occupies.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum FaceCorner {
    /// (u=0, v=0) — bottom-left.
    BottomLeft,
...
///
/// Uses the face's basis vectors (tangent, bitangent, normal) to determine
/// which of the 8 cube corners corresponds to the given UV corner on the face.
#[must_use]
...
///
/// Returns the chunk at the UV corner of `face` that corresponds to `corner`.
#[must_use]
pub fn corner_chunk_on_face(face: CubeFace, corner: CubeCorner, lod: u8) -> ChunkAddress {
    ...
    // Project the corner position onto this face's tangent/bitangent axes
    let s = (corner_pos - n).dot(t);
    let r = (corner_pos - n).dot(b);
...
/// Check if a corner has a valid LOD configuration (difference ≤ 1
/// between any two of the three chunks meeting at the corner).
#[must_use]
pub fn corner_lod_valid(
    ...
    let max_lod = lods.iter().copied().max().unwrap_or(0);
    let min_lod = lods.iter().copied().min().unwrap_or(0);
    max_lod - min_lod <= 1
}
...
impl ChunkAddress {
    ...
    pub fn cube_corner(&self) -> Option<(FaceCorner, CubeCorner)> {
        ...
        let face_corner = match (at_left || at_right, at_bottom || at_top) {
            (true, true) => match (at_left, at_bottom) {
    ...
    pub fn corner_neighbors(&self) -> Option<CornerNeighbors> {
        let (_face_corner, cube_corner) = self.cube_corner()?;
        let faces = cube_corner.faces();

        // Find the two other faces (not self.face)
        let mut other_faces = [CubeFace::PosX; 2];
        let mut idx = 0;
        for &f in &faces {
            if f != self.face {
                other_faces[idx] = f;
    ...
    fn test_corner_chunk_on_face_is_at_corner() {
        for corner in CubeCorner::ALL {
            ...
            for &face in &faces {
                ...
                assert!(
                    (addr.x == 0 || addr.x == grid - 1) && (addr.y == 0 || addr.y == grid - 1),
