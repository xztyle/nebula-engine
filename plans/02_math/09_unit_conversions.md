...
The engine's internal coordinate system uses i128 millimeters, but humans think in meters, kilometers, astronomical units, and light-years. Game designers, debug overlays, configuration files, and serialization formats all need human-readable units. A centralized set of constants and conversion functions prevents magic numbers from being scattered across the codebase and ensures astronomical constants are defined once, correctly.

## Solution

Define constants and functions in a `units` module within `nebula_math`:
...
- All constants are `i128` to match the `WorldPosition` field type, avoiding casts at every call site.
- Conversion functions use `f64` on the human-readable side since game designers and config files use decimal numbers.
- The `round()` call in `meters_to_units` etc. ensures the nearest millimeter is chosen, not truncated.
- Astronomical constants follow IAU definitions where exact values exist.
- The `UNITS_PER_INCH` constant rounds 25.4 to 25 for integer compatibility. If exact inch conversion is needed, use `(inches * 25.4).round() as i128` directly.

### Range verification

The maximum representable distance in the engine:
...
In km:     ~1.70×10³² km
In ly:     ~1.80×10¹⁶ ly
In parsec: ~5.51×10¹⁵ pc
```

The observable universe has a radius of ~46.5 billion light-years (4.65×10¹⁰ ly). The engine's range exceeds this by a factor of ~387 million, so the entire observable universe fits comfortably.

## Outcome

After this story is complete:

- Centralized, well-documented conversion constants exist for all needed units
- Conversion functions are available for meters, km, AU, and light-years
- A human-readable distance formatter is available for debug UI and logging
- No magic numbers need to appear in game code
- Astronomical constants are verified against IAU standards

## Demo Integration

**Demo crate:** `nebula-demo`

The title shows the position in multiple units: `1,666 mm = 1.67 m = 0.002 km`. The conversion utilities are visually validated each tick.
