...
Terrain in a voxel planet engine must look natural: rolling hills, jagged mountain ridges, subtle plains undulation, and continental-scale elevation variation. A single layer of noise produces monotonous, "blobby" landscapes with no sense of scale hierarchy. Real terrain exhibits features at many spatial frequencies simultaneously -- broad continental shelves, medium-scale mountain ranges, and fine-grained rocky detail. The engine needs a noise function that composites multiple frequency layers (octaves) into a single height value, is deterministic for a given seed and coordinate, and is efficient enough to evaluate millions of times during chunk generation. The input coordinate is a 2D position on a cube face (which will later be projected to the sphere surface), and the output is a scalar height in engine units.

## Solution

Implement a `HeightmapSampler` in the `nebula-terrain` crate that generates terrain height values using fractal Brownian motion (fBm) over simplex noise. Each sample composites multiple octaves of noise, where each successive octave doubles in frequency and halves in amplitude, producing self-similar detail at progressively finer scales.
...
For each sample coordinate `(x, y)`:

1. Initialize `total = 0`, `frequency = base_frequency`, `amplitude = params.amplitude`.
2. For each octave `i` in `0..octaves`:
   a. Compute `noise_val = simplex_2d(x * frequency, y * frequency)`.
   b. Accumulate: `total += noise_val * amplitude`.
   c. Scale for the next octave: `frequency *= lacunarity`, `amplitude *= persistence`.
3. Return `total`.

The output range is bounded by the geometric sum of amplitudes: `amplitude * (1 - persistence^octaves) / (1 - persistence)`. With defaults (amplitude = 4000, persistence = 0.5, 6 octaves), the max absolute value is approximately 7875 engine units.

### 3D Overload for Sphere Sampling

For use with cubesphere terrain (story 03), an overloaded `sample_3d` method accepts a `DVec3` point on the unit sphere:
...
A `HeightmapSampler` struct and `HeightmapParams` config in `nebula-terrain` that produces deterministic, multi-octave fractal noise height values from 2D or 3D coordinates. Running `cargo test -p nebula-terrain` passes all heightmap tests. The sampler is usable by the cubesphere terrain system (story 03) and the biome system (story 02).

## Demo Integration

**Demo crate:** `nebula-demo`

Terrain chunks are generated with multi-octave noise producing natural-looking mountains and valleys. The console logs `Heightmap: 6 octaves, range [-128, 128]`.

## Crates & Dependencies

| Crate | Version | Purpose |
|-------|---------|---------|
| `noise` | 0.9 | Simplex and Perlin noise generation with seedable RNGs |
| `glam` | 0.29 | `DVec3` for 3D sphere-surface coordinate sampling |

No other external dependencies. The crate uses Rust edition 2024.
...
mod tests {
    ...
    fn test_determinism_same_seed_same_coord() {
        let params = HeightmapParams { seed: 42, ..Default::default() };
    ...
    fn test_height_within_expected_range() {
        ...
        // Sample a grid of points and ensure all are within bounds.
    ...
    fn test_more_octaves_adds_detail() {
        // With 1 octave, the output is smoother (lower variance at small scales).
        // With 8 octaves, the output has more high-frequency variation.
        // We measure this by computing the average absolute difference between
        // adjacent samples -- more octaves should produce a larger difference.
        let step = 0.5;
    ...
    fn test_smooth_gradient_no_discontinuities() {
        // Adjacent samples at a very small step should have a small difference.
        // A discontinuity would show as a sudden large jump.
        let sampler = HeightmapSampler::new(HeightmapParams {
            seed: 42,
            ..Default::default()
        });
        let step = 0.01;
        let max_allowed_delta = sampler.max_amplitude() * 0.1; // 10% of max per step
    ...
    fn test_max_amplitude_calculation() {
        ...
        let sampler = HeightmapSampler::new(params);
        // Geometric sum: 1000 + 500 + 250 + 125 = 1875
        let expected = 1875.0;
        assert!(
