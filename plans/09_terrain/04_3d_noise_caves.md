...
A voxel planet with only surface terrain is hollow and lifeless underground. Players expect explorable cave systems beneath the surface -- winding tunnels, caverns, and underground passages. Unlike heightmap-based terrain which operates in 2D, caves require a 3D noise field that carves void space into the solid subsurface volume. The cave generation must be deterministic, controllable (density, depth range, connectivity), and must not breach the ocean floor (which would cause visual and physical artifacts with water draining into the underworld). The "Swiss cheese" model -- where 3D noise fields create worm-like connected voids -- produces the most natural-looking cave networks.

## Solution

Implement a `CaveCarver` in the `nebula-terrain` crate that uses multi-octave 3D simplex noise to determine which subsurface voxels are air (cave) versus solid. The system samples a 3D noise field at each voxel position: voxels where the noise exceeds a configurable threshold are solid; voxels at or below the threshold are carved to air.
...
The multi-octave 3D simplex noise naturally produces the "Swiss cheese" pattern. At the default threshold of -0.15, approximately 20-30% of subsurface volume becomes cave. The worm-like appearance emerges because 3D simplex noise creates smoothly connected regions of similar value, and thresholding these regions at a negative value selects the narrow "valleys" of the noise field, which tend to form elongated, tunnel-like shapes.

For more pronounced worm-like tunnels, a second noise field can be multiplied with the first (intersection of two noise thresholds produces thinner, more tunnel-like voids):

```rust
/// Enhanced worm-cave sampling: intersection of two offset noise fields.
fn sample_worm_cave_noise(&self, pos: glam::DVec3) -> f64 {
    let n1 = self.sample_cave_noise(pos);
    // Offset the second sample to decorrelate.
    let offset = glam::DVec3::new(1000.0, 2000.0, 3000.0);
    let n2 = self.sample_cave_noise(pos + offset);
    // Both must be below threshold for a cave to form.
    n1.max(n2)
}
```
...
A `CaveCarver` struct and `CaveConfig` in `nebula-terrain` that deterministically carves cave systems into the subsurface volume using 3D noise. The carver respects depth bounds and ocean floor buffers. Running `cargo test -p nebula-terrain` passes all cave generation tests.

## Demo Integration

**Demo crate:** `nebula-demo`

Underground cavities are visible when slicing through terrain. The debug view shows cave networks carved through solid rock.

## Crates & Dependencies

| Crate | Version | Purpose |
|-------|---------|---------|
| `noise` | 0.9 | 3D simplex noise for cave field generation |
...
mod tests {
    ...
    fn test_cave_voxels_are_air() {
        ...
        // Test many subsurface points. Any that `is_cave` returns true for
        // should be carved as air.
        let mut found_cave = false;
        for i in 0..1000 {
            let depth = 10.0 + (i as f64) * 0.5; // 10 to 510 below surface
            let voxel_pos = DVec3::new(surface_height - depth, 0.0, 0.0);
            if carver.is_cave(voxel_pos, surface_height, sea_level) {
                found_cave = true;
                // The test passes: is_cave=true means this voxel is air.
            }
        }
        assert!(found_cave, "Should find at least some cave voxels in 1000 samples");
    }
    ...
    fn test_non_cave_voxels_are_solid() {
        ...
        assert!(found_solid, "Should find solid (non-cave) voxels in subsurface");
    }
    ...
    fn test_cave_density_controllable_via_threshold() {
        ...
        let caves_low_threshold = count_caves(-0.5);  // very restrictive
        let caves_high_threshold = count_caves(0.2);   // very permissive
    ...
    fn test_deterministic_with_seed() {
        let carver_a = CaveCarver::new(CaveConfig { seed: 123, ..Default::default() });
        let carver_b = CaveCarver::new(CaveConfig { seed: 123, ..Default::default() });
    ...
    fn test_caves_dont_breach_ocean_floor() {
        let carver = CaveCarver::new(CaveConfig {
            seed: 42,
            threshold: 0.5, // Very aggressive carving to maximize chance of breach.
            ocean_floor_buffer: 10.0,
            ..Default::default()
        });

        // Surface is below sea level (ocean floor).
        let surface_height = PLANET_RADIUS - 50.0; // 50 units below sea level
        let sea_level = PLANET_RADIUS;

        // Check voxels just below the ocean floor.
        for i in 0..20 {
            let depth = (i as f64) + 1.0; // 1 to 20 below ocean floor surface
            let voxel_pos = DVec3::new(surface_height - depth, 0.0, 0.0);
            if depth < 10.0 {
                assert!(
                    !carver.is_cave(voxel_pos, surface_height, sea_level),
    ...
    fn test_no_caves_in_eggshell_zone() {
        let carver = CaveCarver::new(CaveConfig {
            seed: 42,
            threshold: 0.5, // Aggressive carving.
            min_depth: 5.0,
            ..Default::default()
        });
        ...
        // Voxels within min_depth of the surface should never be caves.
        for i in 0..50 {
            let depth = (i as f64) * 0.1; // 0.0 to 4.9
